...
##### ON RECENTRE LE KERNEL...    
        if (NbLigne%2 == 0):
            kernel = np.roll(kernel.reshape(NbLigne*NbColonne),int(NbLigne*NbColonne/2-NbLigne/2)).reshape((NbLigne,NbColonne))
        else :
            kernel = np.roll(kernel.reshape(NbLigne*NbColonne),-int(np.floor(NbLigne*NbColonne/2))).reshape((NbLigne,NbColonne))
...
##### METHODE INTERMEDIAIRE
        ...
        for i in range(0,NbLigne*NbColonne) :
            for j in range(0,NbLigne*NbColonne) : 
                ...
                if( (abs(indice_j-i%NbColonne) <= NbColonne/2) and (abs(indice_i-floor(i/NbColonne)) <= NbLigne/2) ): 
...
##### ON CRÉÉ UNE GAUSSIENNE CENTRÉE ET UNITAIRE POUR "kernel"  
        for i in range(0,NbLigne) :
            for j in range(0,NbColonne) :
                kernel[i,j] = 255*e**(-((i-NbLigne/2)*(i-NbLigne/2)/(NbLigne*NbLigne/256))-((j-NbColonne/2)*(j-NbColonne/2)/(NbColonne*NbColonne/256)))
...
##### ON DÉFINIT "image" ET "kernel" ET LA TAILLE
        ...
        kernel = zeros((NbLigne, NbColonne))
        kernel[0,0] = 255; kernel[1,0] = 200; kernel[2,0] = 70; kernel[2,1] = 150
        kernel = kernel/sum(kernel)
...
###############################################################################
def Verifications(On_Verifie_les_kernels=0, On_Verifie_A=0, On_Verifie_la_conv=0, On_Verifie_la_deuxieme_conv=0, On_Compare_deux_matrices=0):
    ...
    if (On_Verifie_la_deuxieme_conv == 1):    
        ...
        print("\n \n \n AK - conv(image,PSF) = \n", conv_matrices(A, PSF, "matrice")-conv(image, PSF), "\n \n Moyenne des differences = ",sum(abs(conv_matrices(A, PSF,"matrice")-conv(image, PSF)))/(NbLigne*NbColonne))
    ...
    if (On_Compare_deux_matrices == 1):    
        ...
        for k in range(0,3*NbColonne):
            print(int(floor(k/NbColonne)),"  ",k%NbColonne ,"  D = ",D[int(floor(k/NbColonne)),k%NbColonne],"    F = ",F[int(floor(k/NbColonne)),k%NbColonne])
        for k in range(0,NbLigne*NbColonne):
            for i in range(0,NbLigne):
                for j in range(0,NbColonne):
                    if (D[i,j] == F[int(floor(k/NbColonne)),k%NbColonne]):
                        print("Positions = ",i+j,"  ",int(floor(k/NbColonne)) + k%NbColonne," Ecart =  ",i+j-(int(floor(k/NbColonne)) + k%NbColonne)," N° = ",k," Nb d'élements similaires = ",l,"\n")
                        l = l+1
        ########################################################
